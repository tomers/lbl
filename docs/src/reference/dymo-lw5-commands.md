...
| `ESC V` | `1B 56` | **Unimplemented** | Get engine version (HW/FW/PID block, 34 bytes). | **Useful.** Printer detail “About” (firmware/hardware), support diagnostics, and compatibility checks before enabling speed modes or new job features. |
...
- Device / status: `lbl-device` `dymo_lw` (`ESC A`, `ESC U`)
