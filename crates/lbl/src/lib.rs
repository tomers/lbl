...
//! `lbl-*` binary) into composable flows, the way `gcc` drives `cpp`, `cc1`,
//! `as`, and `ld`. The [`pipeline`] module exposes the chaining logic used by
//! the `lbl` binary's high-level `print` and `preview` flows.
